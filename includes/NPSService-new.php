...
class NPSService {
    ...
    public function crearEncuesta($datos) {
        try {
            $sql = "INSERT INTO encuestas (titulo, descripcion, pregunta, fecha_inicio, fecha_fin, estado, creado_por) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    public function procesarRespuesta($token, $puntuacion, $comentario = '') {
        try {
            $sql = "SELECT d.*, e.* FROM destinatarios d JOIN encuestas e ON d.encuesta_id = e.id WHERE d.token_unico = ? AND d.respondido = FALSE";
            ...
            $sql = "INSERT INTO respuestas (encuesta_id, destinatario_id, puntuacion, comentario, ip_address, user_agent) VALUES (?, ?, ?, ?, ?, ?)";
            $this->db->query($sql, [
    ...
    public function obtenerResultados($encuestaId) {
        try {
            $sql = "SELECT COUNT(*) as total_respuestas, AVG(puntuacion) as promedio, COUNT(CASE WHEN categoria = 'promotor' THEN 1 END) as promotores, COUNT(CASE WHEN categoria = 'pasivo' THEN 1 END) as pasivos, COUNT(CASE WHEN categoria = 'detractor' THEN 1 END) as detractores FROM respuestas WHERE encuesta_id = ?";
            $stmt = $this->db->query($sql, [$encuestaId]);
            $stats = $stmt->fetch();
            $total = $stats['total_respuestas'];
            $nps_score = $total > 0 ? (($stats['promotores'] - $stats['detractores']) / $total) * 100 : 0;
            $sql = "SELECT puntuacion, COUNT(*) as cantidad FROM respuestas WHERE encuesta_id = ? GROUP BY puntuacion ORDER BY puntuacion";
            $stmt = $this->db->query($sql, [$encuestaId]);
            $distribucion = $stmt->fetchAll();
            $sql = "SELECT r.comentario, r.puntuacion, r.fecha_respuesta, d.nombre, d.email FROM respuestas r JOIN destinatarios d ON r.destinatario_id = d.id WHERE r.encuesta_id = ? AND r.comentario IS NOT NULL AND r.comentario != '' ORDER BY r.fecha_respuesta DESC LIMIT 10";
            $stmt = $this->db->query($sql, [$encuestaId]);
    ...
    public function obtenerEncuestas($filtros = []) {
        try {
            $sql = "SELECT e.*, u.nombre as creador, COUNT(d.id) as total_destinatarios, COUNT(CASE WHEN d.respondido = TRUE THEN 1 END) as respuestas_recibidas FROM encuestas e LEFT JOIN usuarios u ON e.creado_por = u.id LEFT JOIN destinatarios d ON e.id = d.encuesta_id WHERE 1=1";
            $params = [];
    ...
    public function validarToken($token) {
        try {
            $sql = "SELECT d.*, e.* FROM destinatarios d JOIN encuestas e ON d.encuesta_id = e.id WHERE d.token_unico = ? AND d.respondido = FALSE AND e.estado = 'activa'";
            $stmt = $this->db->query($sql, [$token]);
            return $stmt->fetch();
        } catch (Exception $e) {
            return false;
